            ...
            success: function(data) {
                ...
                data.forEach(function(row) {
                    html += `<tr>
                        ...
                        <td>${parseFloat(row.opening_balance).toFixed(2)}</td>
                        <td>${parseFloat(row.period_debit).toFixed(2)}</td>
                        <td>${parseFloat(row.period_credit).toFixed(2)}</td>
                        <td>${parseFloat(row.closing_balance).toFixed(2)}</td>
                    </tr>`;
                });
                html += '</tbody></table>';
            ...
            success: function(data) {
                ...
                data.forEach(function(row) {
                    html += `<tr>
                        ...
                        <td style="text-align: right;">${row.asset_opening === null ? '' : parseFloat(row.asset_opening).toFixed(2)}</td>
                        <td style="text-align: right;">${row.asset_closing === null ? '' : parseFloat(row.asset_closing).toFixed(2)}</td>
                        <td style="text-align: left;">${row.liability_equity_item || ''}</td>
                        <td style="text-align: right;">${row.liability_equity_opening === null ? '' : parseFloat(row.liability_equity_opening).toFixed(2)}</td>
                        <td style="text-align: right;">${row.liability_equity_closing === null ? '' : parseFloat(row.liability_equity_closing).toFixed(2)}</td>
                    </tr>`;
                });
                html += '</tbody></table>';
                $displayArea.html(html);
                ...
                data.forEach(function(row) {
                    ...
                    html += `
                            <tr>
                                ...
                                <td style="text-align: right;">${amount === null ? '' : parseFloat(amount).toFixed(2)}</td>
                            </tr>`;
                });
                html += '</tbody></table>';
                $displayArea.html(html);
